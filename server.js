...
app.put("/api/evaluations/:id/status", async (req, res) => {
  ...
  try {
    ...
    const totalQuestions =
      evaluation.questionDistribution.easy +
      evaluation.questionDistribution.medium +
      evaluation.questionDistribution.difficult;
    ...
    if (evaluation.status === "draft" && status === "active") {
      ...
      try {
        ...
        if (evaluation.questionType === "MCQ") {
          ...
          generatedQuestions = await generateDescriptiveQuestions(topicContent, evaluation.topic, totalQuestions);
        }
